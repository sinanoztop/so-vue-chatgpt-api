<template>
  <div id="edit-text-w">
    ...
    <div class="menu-option-title">
      <div class="left">
        <span><em class="fas fa-image"></em></span>
      </div>
      <div class="right">
        <span>Resim Bul</span>
        <p>
          Resimde olmasını istediğiniz kelimeleri aşağıdaki alana yazın ve
    ...
    <div class="menu-option-content">
      <div class="form-item">
        <input
          ...
          v-model="text"
          placeholder="Örneğin; gülen kedi"
        />
        <span class="example-form-text">Örnek: gülen kedi</span>
      </div>
      <div class="form-item">
  ...
  methods: {
    async create() {
      ...
      if (_length == 0) {
        ...
        this.loading = true;
        axios
          .post(process.env.VUE_APP_API_URL + "image/search", _data)
          .then((res) => {
            let _res = res.data;
            setTimeout(() => {
              this.loading = false;
              if (_res.result == "success") {
                this.postRes = true;
                this.postResC = _res.message.data;
              }
